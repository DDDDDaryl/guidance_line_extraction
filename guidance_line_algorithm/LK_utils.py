...
class App:
    def __init__(self, video_src='1', fps=30):  # 构造方法，初始化一些参数和视频路径
        self.track_len = 10
        self.detect_interval = 5
        self.tracks = []    # 角点
        self.cam = cv2.VideoCapture(video_src)
        self.frame_idx = 0
        self.pixel_to_meter_scale = 1 / 600
        self.fps = fps
    ...
    def get_speed(self):
        ...
        if num_tracks == 0:
            return self.speed_cache
        for track in self.tracks:
            if len(track) > 1:
                vec = np.array(track[-1]) - np.array(track[0])
                vec[1] = 0 # 只取水平速度，y方向速度置零
                sum += np.sqrt(np.sum(np.square(vec)))
        sum /= num_tracks
        self.speed_cache = self.pixel_to_meter_scale * sum / ((self.track_len-1)/self.fps)
        return self.speed_cache
...
def main():
    ...
    try:
        ...
        video_src = 'D:\\华农项目资料\\深度学习\\raw\\2020_01_06_17_27_IMG_0869.MOV'
